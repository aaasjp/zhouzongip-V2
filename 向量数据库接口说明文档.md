...
```json
{
    ...
    "tenant_code": "xiaomi",
    "collection_name": "xiaoshou",
    "question": ["1+1等于多少？", "2+2等于多少？", "3+3等于多少？"],
    "answer": ["1+1=2", "2+2=4", "3+3=6"],
    "source": ["data/qa/wiki.txt", "data/qa/wiki.txt", "data/qa/math.txt"]
}
```
...
- `tenant_code` (string, 必填): 租户统一编码
- `collection_name` (string, 必填): 知识库名称
- `question` (string | array, 必填): 问题内容。可以是字符串（单个插入）或字符串数组（批量插入）
- `answer` (string | array, 必填): 答案内容。可以是字符串（单个插入）或字符串数组（批量插入）
- `source` (string | array, 可选): 来源标识。可以是字符串（单个插入时使用，批量插入时会被重复应用到所有问答对）或字符串数组（批量插入时，长度必须与 `question` 和 `answer` 数组长度一致）

**注意事项**:
- `question` 和 `answer` 必须同时为字符串或同时为数组
- 当使用数组时，`question` 和 `answer` 数组长度必须一致
- 当 `source` 为字符串时，批量插入会自动将该值应用到所有问答对
- 当 `source` 为数组时，长度必须与 `question` 和 `answer` 数组长度一致
...
6. **问题列表长度与答案列表长度不一致**: 使用批量插入时，确保 `question` 和 `answer` 数组长度相同
7. **问题参数格式不正确**: `question` 和 `answer` 必须同时为字符串或同时为数组
8. **来源列表长度不一致**: 当 `source` 为数组时，长度必须与 `question` 和 `answer` 数组长度一致
...
7. `add_qa` 接口支持单个和批量插入，`question` 和 `answer` 可以同时为字符串（单个）或数组（批量），但类型必须一致
8. 批量插入时，`question` 和 `answer` 数组长度必须一致，`source` 可以是字符串（自动重复）或数组（长度需匹配）
