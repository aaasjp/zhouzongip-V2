...
```bash
docker run -d \
  --name mysql-ai_helper \
  --restart=unless-stopped \
  -e MYSQL_ROOT_PASSWORD=root123 \
  -e MYSQL_DATABASE=ai_helper \
  -e MYSQL_USER=helper \
  -e MYSQL_PASSWORD=helper123 \
  -e TZ=Asia/Shanghai \
  -p 3306:3306 \
...
# 查看容器日志
docker logs mysql-ai_helper

# 进入容器测试连接（使用utf8mb4字符集，避免中文乱码）
docker exec -it mysql-ai_helper mysql -uroot -proot123 --default-character-set=utf8mb4

# 或者进入后手动设置字符集
docker exec -it mysql-ai_helper mysql -uroot -proot123
...
```sql
-- 创建用户 'helper'，密码为 'helper123'
CREATE USER 'helper'@'%' IDENTIFIED BY 'helper123';

-- 创建数据库 ai_helper
CREATE DATABASE ai_helper CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- 授予该用户对 ai_helper 数据库的全部权限
GRANT ALL PRIVILEGES ON ai_helper.* TO 'helper'@'%';
...
# 构建 Docker 镜像
docker build -t ai-helper:latest -f Dockerfile .

# 查看镜像
docker images | grep ai-helper
```
...
```bash
# 查看实时日志
docker logs -f ai-helper

# 查看最近 100 行日志
docker logs --tail 100 ai-helper
```
...
```bash
# 检查容器运行状态
docker ps | grep ai-helper

# 检查服务健康状态
curl http://localhost:8005/health  # 如果有健康检查接口

# 检查服务日志中是否有错误
docker logs ai-helper | grep -i error
```
...
```bash
# 测试向量库服务（示例）
curl -X POST http://localhost:8005/vector_db_service/health

# 测试对话服务（示例）
curl -X POST http://localhost:8005/chat_service/health
```
...
3. 检查防火墙是否开放 3306 端口
4. 验证 MySQL 用户权限: `GRANT ALL PRIVILEGES ON ai_helper.* TO 'helper'@'%';`
...
**解决方案**:
1. 确认模型目录已正确映射: `docker exec ai-helper ls -la /workspace/embeddings/`
2. 检查模型文件完整性
...
3. 或修改 Docker 启动命令中的端口映射: `-p 8006:8005`
...
3. 确保数据库创建时指定了字符集: `CREATE DATABASE ai_helper CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;`
...
**解决方案**:
1. 查看详细日志: `docker logs ai-helper`
